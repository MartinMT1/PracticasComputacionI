...
int CalcularCargaApH(float& ph, grupos todosGrupos)
{
    for (int i = 0; i < todosGrupos.size(); i++) { // Hacemos una iteración en en los grupos agregados
        if (todosGrupos[i].ObtenerPkr() > ph ) {//Cuando el valor en i correspondiente al pkr es mayor al ph la carga será  el valor guardado en carga + 1
            Carga = todosGrupos[i].ObtenerCarga() + 1;
        }
        if (todosGrupos[i].ObtenerPkr() < ph) {// En cambio, cuando el valor de ph sea menor al valor en i correspondiente al pkr
            Carga = todosGrupos[i].ObtenerCarga();// la carga será el valor guardado en memoria
        }
        CargaDeMolecula = CargaDeMolecula + Carga; 
    }
    return CargaDeMolecula;
}
